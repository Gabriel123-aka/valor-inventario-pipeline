...
# =========================================================
# ======= TU CÓDIGO COMPLETO (CON TODAS LAS MEJORAS INTEGRADAS) =======
import pandas as pd
import os
from pathlib import Path
import datetime
# Imports consolidados al inicio para evitar errores de acceso local o variables no asociadas
...
# =====================================================
# CONFIGURACIÓN PORTABLE (GITHUB Y LOCAL)
# =====================================================
# Detectamos la ubicación del proyecto para crear rutas relativas
BASE_DIR = Path(__file__).resolve().parent.parent

# Ruta de ORIGEN: Si no existe la red, usa 'data_samples'
UNC_FOLDER = r"\\192.168.1.195\odbc_dir\Planeacion"
if not os.path.exists(UNC_FOLDER):
    UNC_FOLDER = str(BASE_DIR / "data_samples")
    print(f"⚠️ Usando datos locales de: {UNC_FOLDER}")

PREFIXES = ["Inventario", "TransitosPendientes", "DOH_C", "OCPendiente", "Entradas X Planeacion"]
EXTS = [".xlsx", ".xls", ".csv"]

# Ruta de DESTINO: Si no existe la red, usa una carpeta 'output'
CARPETA_DESTINO = r"\\192.168.11.1\Planeacion\PublicaInventario"
if not os.path.exists(CARPETA_DESTINO):
    CARPETA_DESTINO = str(BASE_DIR / "output")
    os.makedirs(CARPETA_DESTINO, exist_ok=True)
    print(f"📂 Resultados guardados en: {CARPETA_DESTINO}")
...
OBJETIVO_CONSTANTE = 1875000000  # Se puede modificar en el futuro
...
def main():
    ...
    if df_inventario is not None and os.path.exists(ARCHIVO_VALOR_INVENTARIO):
        try:
            ...
            col_fecha = datetime.datetime.now().strftime("%d/%m/%Y")
